...
public class Register extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{                
                int password = Integer.parseInt(request.getParameter("y"));
                double balance = Double.parseDouble(request.getParameter("z"));
                MyCustomer found = myCustomerFacade.find(name);
                if(found!=null){
                    throw new Exception();
                }
                if(balance<=0){
                ...
                myCustomerFacade.create(new MyCustomer(name,password,balance));
